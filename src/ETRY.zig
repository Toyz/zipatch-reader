...
pub const ChunkMode = enum(u32) {
    ...
    /// Creates a ChunkMode from a u32 value
...
pub const CompressionMode = enum(u32) {
    ...
    /// Creates a CompressionMode from a u32 value
    /// value: The u32 value to convert
...
pub const Chunk = struct {
    ...
    pub fn parseFromBytes(bytes: []const u8, allocator: mem.Allocator) !Chunk {
    ...
    fn decompressZlib(self: *const Chunk, allocator: Allocator) ![]u8 {
        ...
        var decompress = std.compress.flate.decompressor(deflate_stream.reader());
...
fn createParentDirectories(file_path: []const u8) !void {
    const dir_path = fs_path.dirname(file_path) orelse return;
...
pub const Etry = struct {
    ...
    pub fn parseFromBytes(bytes: []const u8, allocator: mem.Allocator) !Etry {
        var offset: usize = 0;
        var temp_buffer_u32: [4]u8 = undefined;
